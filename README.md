...
A rule-based prompt optimizer built in pure Java — no AI APIs, no external dependencies.
Type a rough prompt, and the tool compresses it, measures the token savings, and restructures it into something that gets better responses from AI assistants like ChatGPT or Claude.
...
BetterPrompt compresses your prompt using a strategy you choose, shows you how many tokens were saved, then wraps the result in a structured template — entirely with Java logic, no API calls required.
...
V2 runs a six-step pipeline every time you enter a prompt:
...
Steps 1–3 are new in V2. Steps 4–6 are carried over from V1.
...
## The Three Domains (V1, still active)
...
│   └── Main.java                      # Terminal loop — updated for V2
│
...
**Goal: Build a web UI, upgrade compression with open-source corpora, and measure whether compression is actually worth it.**

### 1. Web UI Framework
Build a simple web interface using Spring Boot + HTML:
- Input box for user's question
- Dropdown or buttons to select compression strategy
- Side-by-side display of original vs compressed prompt
- Token count comparison
- Designed to be extensible: new strategies can be added without changing the UI

### 2. Open-Source Corpus Integration
Replace the small hardcoded keyword lists in V2 with large open-source corpora:
- Filler phrases corpus (from academic NLP research)
...
### 4. Efficiency Index
A single metric to determine if compression is worth the cost:
